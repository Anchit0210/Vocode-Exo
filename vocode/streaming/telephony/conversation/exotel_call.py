...
from vocode.streaming.utils.state_manager import ExotelCallStateManager
...
class ExotelCall(Call[ExotelOutputDevice]):
    def __init__(
        self,
        from_phone: str,
        to_phone: str,
        ...
        self.exotel_config = exotel_config
        self.telephony_client = ExotelClient(
            base_url=base_url, exotel_config=self.exotel_config
        )
        super().__init__(
            from_phone,
            to_phone,
    ...
    def create_state_manager(self) -> ExotelCallStateManager:
        return ExotelCallStateManager(self)

    async def attach_ws_and_start(self, ws: WebSocket):
        super().attach_ws(ws)
        await self.wait_for_exotel_start(ws)
        await super().start()
    ...
    async def wait_for_exotel_start(self, ws: WebSocket):
