...
class TelephonyServer:
    ...
    def create_inbound_route(
        ...
        async def exotel_route(
            ...
            self.logger.info(f"Call connected from: {exotel_from}  --->  {exotel_to}")
            client_specific_config = await self.config_manager.get_client_config()
            if not isinstance(client_specific_config, ExotelConfig):
                await self.add_route_async(inbound_call_config)

            call_config = ExotelCallConfig(
                transcriber_config=client_specific_config.transcriber_config
                                   or ExotelCallConfig.default_transcriber_config(),
                agent_config=client_specific_config.agent_config,
                synthesizer_config=client_specific_config.synthesizer_config
                                   or ExotelCallConfig.default_synthesizer_config(),
                exotel_config=client_specific_config.exotel_config,
        ...
        if isinstance(inbound_call_config, TwilioInboundCallConfig):
            ...
            logger.info(
                f"Set up inbound call at https://{self.base_url}{inbound_call_config.url}"
            )
